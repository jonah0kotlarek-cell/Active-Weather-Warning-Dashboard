...
async function fetchWarnings() {
  try {
    ...
    const [codItems, warningSets] = await Promise.all([
      ...
      Promise.all(WARN_URLS.map(fetchWithPagination)),
    ]);
    const capRes = '';
...
async function fetchMdMeta(md) {
  ...
  try {
    ...
    if (md.text !== text || md.watchProb !== watchProb || md.concerning !== concerning) {
...
function queueMdMetaFetch(md) {
  if (!md || md.watchProb || mdMetaLoading.has(md.id)) return;
  void fetchMdMeta(md);
}
