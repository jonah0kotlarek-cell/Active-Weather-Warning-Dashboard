...
function normalizeCountyName(name) {
  return String(name || '')
    .toUpperCase()
    .replace(/\b(PARISH|COUNTY|BOROUGH|CENSUS AREA|CITY AND BOROUGH|MUNICIPALITY|CITY)\b/g, '')
    .replace(/\b[A-Z]{2}\b$/g, '')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function countyNames(area) {
  return String(area || '')
    .split(',')
    .map(part => normalizeCountyName(part))
    .filter(Boolean);
}

function countyOverlapScore(a, b) {
  const left = new Set(countyNames(a));
  const right = new Set(countyNames(b));
  if (!left.size || !right.size) return 0;
  let overlap = 0;
  left.forEach(name => {
    if (right.has(name)) overlap += 1;
  });
  return overlap;
}
...
function enrichCodWarning(codWarning, nwsWarnings) {
  ...
  const matches = nwsWarnings
    ...
    .filter(candidate => candidate.overlap > 0 || candidate.timeDelta <= 10 * 60_000)
...
function classifyCodWarning(item) {
  const type = String(item?.warn_type || '').toUpperCase();
  const tornado = String(item?.tornado || '').toUpperCase();

  if (type === 'TORNADO') {
    return {
      type: 'Tornado Warning',
      variant: tornado.includes('OBSERVED') ? 'observed' : 'base',
    };
  }
...
function parseCodWarnings(items) {
  ...
  for (const item of items) {
    ...
    const issued = item.time_begin_ts ? new Date(Number(item.time_begin_ts) * 1000) : new Date();
    const expires = item.time_end_ts ? new Date(Number(item.time_end_ts) * 1000) : new Date(Date.now() + 3_600_000);
    ...
    out.push({
      id: productUrl || `${item.office || 'cod'}-${item.warn_type || 'warn'}-${item.time_begin_ts || Date.now()}-${counties.join(',')}`,
      type: cl.type,
      variant: cl.variant,
      cfg,
      issued,
      expires,
      area: counties.join(', ') || 'Unknown Area',
      officeCode: String(item.office || '').toUpperCase(),
      radarId: '',
      center: null,
      headline: `${cfg.label} until ${fmtTime(expires)}`,
      desc: productUrl ? `PRODUCT: ${productUrl}` : '',
      instruction: '',
      rank: cfg.rank,
      hailSize: item.hail_size ? String(item.hail_size) : null,
      windGust: item.wind_speed ? String(item.wind_speed) : null,
      damageThreat: null,
      torDetect: tornadoText && tornadoText.toUpperCase() !== 'POSSIBLE' ? tornadoText : null,
      torPossible: tornadoText.toUpperCase().includes('POSSIBLE'),
      source: 'cod',
    });
  }
...
function parseMDs(data) {
  ...
  return data.map(md => ({
    ...
    peakIntensity: md.peak_intensity || null,
...
async function fetchWithPagination(startUrl) {
  let allFeatures = [];
  let url = startUrl;
  while (url) {
...
async function fetchWarnings() {
  try {
    ...
    const previousWarningsByKey = new Map(warnings.map(w => [warningIdentityKey(w), w]));
    const previousWarningKeys = new Set(warnings.map(warningIdentityKey));
    const [codRes, nwsSets] = await Promise.all([
      fetch(COD_WARN_URL, {
        headers: { 'User-Agent': 'GR-Warnings-Desktop/1.0' },
      }),
      Promise.all(NWS_WARN_URLS.map(fetchWithPagination)).catch(() => []),
    ]);
    if (!codRes.ok) throw new Error(`HTTP ${codRes.status}`);
    const data = await codRes.json();
    const parsed = parseCodWarnings(data);
    const nwsFeatures = Array.isArray(nwsSets)
      ? nwsSets.flatMap(set => set?.features || [])
      : [];
    const nwsWarnings = parseWarnings({ features: nwsFeatures }).map(w => ({ ...w, source: 'api' }));
    const enriched = parsed.map(warning => enrichCodWarning(warning, nwsWarnings));
    warnings = enriched.map(w => {
      const previous = previousWarnings.get(w.id) || previousWarningsByKey.get(warningIdentityKey(w));
      return {
        ...w,
        radarId: w.radarId || previous?.radarId || '',
        center: w.center || previous?.center || null,
        isNew: !knownWarnIds.has(w.id) && !previousWarningKeys.has(warningIdentityKey(w)) && (Date.now() - w.issued.getTime() < 90_000),
      };
    });
    knownWarnIds = new Set(enriched.map(w => w.id));
...
async function fetchWatches() {
  try {
    ...
    const data   = await fetchWithPagination(WATCH_URL);
  ...
  try {
    ...
    mds = parsed.map(m => {
      ...
      return {
        ...
        peakIntensity: existing ? existing.peakIntensity : null,
      };
    });
...
function playSpecialWarningSounds(previousWarnings, nextWarnings) {
  ...
  for (const warning of nextWarnings) {
    ...
    if (!isNewArrival && !upgradedToTarget) continue;

    if (warning.type === 'Tornado Warning' && ['pds', 'emergency'].includes(warning.variant)) {
      soundedWarningKeys.add(soundKey);
      playAudio(TORE_SOUND_URL);
      continue;
    }
    if (warning.type === 'Severe Thunderstorm Warning' && warning.variant === 'destructive') {
      soundedWarningKeys.add(soundKey);
...
function parseMDPeakIntensity(text) {
  const source = String(text || '');
  const match = source.match(/\bMOST PROBABLE PEAK INTENSITY\.\.\.([^\n\r]+)/i)
    || source.match(/\bPEAK INTENSITY\.\.\.([^\n\r]+)/i)
    || source.match(/\bMOST PROBABLE PEAK INTENSITY:\s*([^\n\r]+)/i)
    || source.match(/\bPEAK INTENSITY:\s*([^\n\r]+)/i);
...
function parseWatchLikelihoods(html) {
  const doc = new DOMParser().parseFromString(html, 'text/html');
  const valueCells = [...doc.querySelectorAll('td.wwprb-verylow, td.wwprb-low, td.wwprb-moderate, td.wwprb-high')];
  if (valueCells.length < 6) return null;
  ...
  const values = valueCells.slice(0, 6).map(cell => cell.textContent.replace(/\s+/g, ' ').trim());
  return labels.map((label, index) => ({ label, value: values[index] || 'Unknown' }));
}
...
async function fetchWatchMeta(watch) {
  ...
  try {
    ...
    const likelihoods = parseWatchLikelihoods(html);
    let changed = false;
    ...
    if (JSON.stringify(watch.likelihoods || null) !== JSON.stringify(likelihoods || null)) {
      watch.likelihoods = likelihoods;
      changed = true;
    }
    if (changed) renderWatches();
  } catch (e) {
...
function queueWatchMetaFetch(watch) {
  if (!watch || !watch.watchNumber || watchMetaLoading.has(watch.id)) return;
...
async function fetchMdMeta(md) {
  if (!md?.url) return;
  mdMetaLoading.add(md.id);
  try {
    ...
    const concerning = parseMDConcerning(text);
    const watchProb = parseMDWatchProb(text);
    const peakIntensity = parseMDPeakIntensity(text);
    if (md.text !== text || md.watchProb !== watchProb || md.concerning !== concerning || md.peakIntensity !== peakIntensity) {
      md.text = text || md.text;
      md.concerning = concerning;
      md.watchProb = watchProb;
      md.peakIntensity = peakIntensity;
      renderMDs();
    }
  } catch (e) {
    console.warn(`MD meta fetch error (${md.id}):`, e.message);
  } finally {
    mdMetaLoading.delete(md.id);
  }
}

function queueMdMetaFetch(md) {
  if (!md || (md.watchProb && md.peakIntensity) || mdMetaLoading.has(md.id)) return;
  void fetchMdMeta(md);
}
...
async function fetchWarningRadar(warning) {
  ...
  const lat = roundCoord(warning.center.lat);
  const lon = roundCoord(warning.center.lon);
...
function warningRenderHash() {
  return warnings.map(w => `${w.id}:${w.radarId || ''}:${minsLeft(w.expires)}:${w.isNew ? 1 : 0}:${expanded['w_' + w.id] ? 1 : 0}`).join(',');
}

function watchRenderHash() {
  return watches.map(w => `${w.id}:${w.variant}:${minsLeft(w.expires)}:${w.isNew ? 1 : 0}:${expanded['wt_' + w.id] ? 1 : 0}:${(w.likelihoods || []).map(item => `${item.label}:${item.value}`).join('|')}`).join(',');
}

function mdRenderHash() {
  return mds.map(m => `${m.id}:${m.isNew ? 1 : 0}:${expanded['md_' + m.id] ? 1 : 0}:${m.concerning || ''}:${m.watchProb || ''}:${m.peakIntensity || ''}:${m.text}`).join(',');
}

function reportRenderHash() {
  return spotterReports.map(r => `${r.id}:${r.isNew ? 1 : 0}:${expanded['rp_' + r.id] ? 1 : 0}`).join(',');
}
...
function buildWatchRow(w) {
  ...
  const bodyHtml = isExp ? (() => {
    const desc = (w.desc || '').slice(0, 3000);
    const inst = (w.instruction || '').slice(0, 1500);
    const likelihoodHtml = Array.isArray(w.likelihoods) && w.likelihoods.length
      ? `<div style="margin-bottom:6px;border-bottom:1px solid #1a1a1a;padding-bottom:6px;color:#f2f2f2;">${w.likelihoods.map(item => `<div>${esc(item.label.toUpperCase())}...${esc(item.value.toUpperCase())}</div>`).join('')}</div>`
      : '';
    return `
  ...
  const bodyHtml = isExp ? `
    <div class="md-body">
      <div class="md-body-inner">
        ...
        ${md.peakIntensity ? `<div style="color:#ffcc66;margin-bottom:5px;">MOST PROBABLE PEAK INTENSITY: ${esc(md.peakIntensity.toUpperCase())}</div>` : ''}
  ...
  return `
    <div class="md-row" data-id="${esc(md.id)}" data-kind="md" style="${rowAnim}">
      <div class="md-header">
        ...
        ${md.peakIntensity ? `<div class="md-watch-prob">PEAK INTENSITY: ${esc(md.peakIntensity.toUpperCase())}</div>` : ''}
      </div>
      ${bodyHtml}
    </div>`;
}
...
function renderWatches() {
  ...
  const hash = watchRenderHash();
  if (hash === lastWatchHash) return;
  lastWatchHash = hash;

  const scrolls = saveScrolls(list);
  ...
  list.innerHTML = html;
  restoreScrolls(list, scrolls);
  attachClickHandlers(list);
}

function renderMDs() {
  const list = document.getElementById('md-list');
  const hash = mdRenderHash();
  if (hash === lastMdHash) return;
  lastMdHash = hash;
...
function renderReports() {
  ...
  const hash = reportRenderHash();
  if (hash === lastReportHash) return;
  lastReportHash = hash;
...
setInterval(() => { if (warnings.length > 0 || watches.length > 0 || mds.length > 0 || spotterReports.length > 0) renderAll(); }, 30_000);
